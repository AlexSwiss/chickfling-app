...
module.exports = {
    getUserSeed: async() => {
        ...
        for (var k=0;k<body.results.length;k++) {
            ...
            request(`https://maps.googleapis.com/maps/api/geocode/json?address=${body.results[k].location.city.charAt(0).toUpperCase()+body.results[k].location.city.substring(1)}&key=AIzaSyCdr8vf-BHgnuGQOUUNkVoXzhSDMdB9gQw`, async (err, resp, body) => {
                body = JSON.parse(body);
